...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "UPDATE grainscereals SET foodName='$foodName', location_stored='$locationStored', notes='$notes', expiration_date='$expirationDate', shelf_life='$shelfLife' WHERE foodID=$foodID";

    if ($conn->query($sql) === TRUE) {
        header('Location: /PROJECTS/FOODWEB/GRAINSCEREALS/grainscereals.php');
        exit();
    } else {
    ...
    // Retrieve the record
    $foodID = $_GET['id'];
    $sql = "SELECT * FROM grainscereals WHERE foodID=$foodID";
...
<body>
    <div class="container">
        ...
        <form action="editgrainscereals.php" method="post" onsubmit="return validateForm()">
            ...
            <button onclick="window.location.href='/PROJECTS/FOODWEB/GRAINSCEREALS/grainscereals.php';" class="back-button">Back</button>
            </div>
        </form>
    </div>
