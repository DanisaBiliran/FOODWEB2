...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "UPDATE sweets SET foodName='$foodName', location_stored='$locationStored', notes='$notes', expiration_date='$expirationDate', shelf_life='$shelfLife' WHERE foodID=$foodID";

    if ($conn->query($sql) === TRUE) {
        header('Location: /PROJECTS/FOODWEB/SWEETS/sweets.php');
        exit();
    } else {
    ...
    // Retrieve the record
    $foodID = $_GET['id'];
    $sql = "SELECT * FROM sweets WHERE foodID=$foodID";
...
<body>
    <div class="container">
        ...
        <form action="editSweets.php" method="post" onsubmit="return validateForm()">
            ...
            <button onclick="window.location.href='/PROJECTS/FOODWEB/SWEETS/sweets.php';" class="back-button">Back</button>
            </div>
        </form>
    </div>
